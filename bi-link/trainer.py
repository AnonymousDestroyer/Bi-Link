...
class Trainer:
    ...
    def __init__(self, args):
        ...
        self.optimizer_forward = AdamW([p for p in self.model.head_bert.parameters() if p.requires_grad],
                               lr=args.lr,
                               weight_decay=args.weight_decay)
        self.optimizer_backward = AdamW([p for p in self.model.tail_bert.parameters() if p.requires_grad],
                               lr=args.lr,
                               weight_decay=args.weight_decay)
        ...
        self.scheduler_forward = self._create_lr_scheduler(self.optimizer_forward, num_training_steps)
        self.scheduler_backward = self._create_lr_scheduler(self.optimizer_backward, num_training_steps)
    ...
    def train_epoch(self, epoch):
        ...
        progress = ProgressMeter(
            ...
            [losses,losses_forward, losses_backward, head_top1, tail_top1, inv_t, hr_tr_scale],
            prefix="Epoch: [{}]".format(epoch))


        for i, batch_dict in enumerate(self.train_loader):
            if torch.cuda.device_count() > 1:
                wandb.watch(self.model.module.head_bert, log_freq=100)
            else:
                wandb.watch(self.model.head_bert, log_freq=100)
            ...
            # log every step
            wandb.log({"losses": losses.avg, "head_acc1": head_top1.avg, "head_acc10": head_top10.avg, "tail_acc1": tail_top1.avg, "inv_t": inv_t.avg})
            wandb.log({"lr": self.scheduler_forward.get_last_lr()[0]})
            add_scalars(self.tensor_logger, {"losses": losses.avg,
                                                                "head_acc1": head_top1.avg,
                                                                "tail_acc1": tail_top1.avg, "inv_t": inv_t.avg,
                                                                "lr": self.scheduler_forward.get_last_lr()[0]
                                                                },
                                           self.step)
            ...
            if self.args.use_amp:
                ...
                self.scaler.unscale_(self.optimizer_forward)
                self.scaler.unscale_(self.optimizer_backward)
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.grad_clip)
                # self.scaler.step(self.optimizer)
                self.scaler.step(self.optimizer_forward)
                self.scaler.step(self.optimizer_backward)
                self.scaler.update()
            else:
                loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.grad_clip)
        ...
        logger.info('Learning rate: {}'.format(self.scheduler_forward.get_last_lr()[0]))
